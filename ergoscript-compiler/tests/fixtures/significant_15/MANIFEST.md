...
|---|---|---|---|---|---|
| `chaincash_reserve.es`           | 611  | 613  | +2   | USED NODE | was -65 → -62 (S68) → -61 (S69 Append arm) → -60 (S70 Exponentiate arm) → **+2 (S71** — `.multiply` lowering + `replace_all` MultiplyGroup arm); chaincash now compiles **with-segregation** (header 0x10 matches), residual +2B is constants-segment encoding drift at byte 12. Same shape as `SigUSDV1` ecosystem residual. |
...
- `chaincash_reserve` (+2) — was -61 fallback through S69/S70, now **+2 with-segregation** post-S71. Root cause was a chain of three coupled bugs: (1) `replace_all` missing Append arm (S69 closed type-collision class); (2) `replace_all` missing Exponentiate arm (S70 substituted dangling `ValUse(26)` under Exponentiate.right via ByteArrayToBigInt → CalcBlake2b256 → Append-chain); (3) `.multiply` lowered to `MethodCall(MULTIPLY_METHOD)` but the parser rejects it because GroupElement.multiply is absent from `METHOD_DESC` — S71 switched the lowering to canonical `Expr::MultiplyGroup` (mirroring `.exp` → `Exponentiate`) and added the `replace_all` MultiplyGroup arm (the lowering switch alone re-exposed `ValDefIdNotFound(26)` since MultiplyGroup wasn't in `replace_all`'s arms either). Header now matches at 0x10; residual +2B is constants-segment encoding drift at byte 12 (separate investigation, S72-class). Known-future-arms in `replace_all` (two removed): LongToByteArray, ByteArrayToLong, ~~Exponentiate~~, ~~MultiplyGroup~~, DecodePoint, GetVar, CreateProveDhTuple, Atleast, BitInversion, CalcSha256, CreateAvlTree, DeserializeContext, DeserializeRegister, ExtractBytesWithNoRef, FuncValue, SubstConstants, Xor, XorOf, ZkProofBlock — each requires its own concrete failure trace per WS-E methodology before addition.
