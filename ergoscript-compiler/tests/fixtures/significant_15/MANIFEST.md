...
|---|---|---|---|---|---|
| `chaincash_reserve.es`           | 611  | 608  | -3   | USED NODE | was -65 → -62 (S68) → -61 (S69) → -60 (S70) → +2 (S71) → -2 (S73) → **-3 (S74** — added missing `LongToByteArray`/`ByteArrayToLong`/`DecodePoint`/`MultiplyGroup`/`Exponentiate` arms to `direct_children` so `count_val_uses_in` correctly counts ValUses buried in those expressions). With S74 fix, `positionBytes = longToByteArray(position)` is correctly preserved as a ValDef in MIR (count==2: in `key`'s Append inlined into `value`'s TreeLookup, and in `message`'s Append inlined into the signature `blake2b256` chain). Without the missing arms, one of those ValUses was hidden, making MIR's `inline_single_use_vals` see count==1 and inline positionBytes. Same bug class as S66b (`ByteArrayToBigInt`) and S68 (`CreateProveDhTuple`). RT-OK preserved, header `0x10` matches. **Residual -3B**: LOCAL still extracts `receiptOut.R6[Int].get` and `receiptOut` itself as inner-block ValDefs; NODE keeps `receiptOut.R6` inline (both uses are right-arm thunks of the `&&` chain, both inside the same outer scope where receiptOut lives). LOCAL has 13 inner items, NODE has 12. The fix made positionBytes correct but exposed receiptOut+R6 as the next bottleneck — the gap widened from -2 to -3 because positionBytes preservation costs 1B more inline (ValDef + 2 ValUses + ValDef header) than the previous bugged inlining. See `10f-chaincash-receiptout-r6-thunk-localization-HANDOFF.md` for S75. |
...
- `chaincash_reserve` (-3) — was -61 → +2 (S71) → -2 (S73) → **-3 (S74** — `direct_children` missing 5 arms). S74 added `LongToByteArray`/`ByteArrayToLong`/`DecodePoint`/`MultiplyGroup`/`Exponentiate` to `direct_children`, fixing `count_val_uses_in` undercounting that wrongly inlined `positionBytes`. positionBytes is now a ValDef matching NODE. Same bug class as S66b/S68. Residual -3B: receiptOut+R6 over-extracted (LOCAL has 13 inner items, NODE 12). NODE leaves receiptOut inline because all uses are in `&&` right-arm thunks; mirroring requires Scala-style thunk-localization at HIR/MIR. Defer to S75 (`10f-chaincash-receiptout-r6-thunk-localization-HANDOFF.md`). SigUSDV1 unchanged. Known-future-arms in `replace_all` (unchanged, separate from `direct_children`): GetVar, CreateProveDhTuple, Atleast, BitInversion, CalcSha256, CreateAvlTree, DeserializeContext, DeserializeRegister, ExtractBytesWithNoRef, FuncValue, SubstConstants, Xor, XorOf, ZkProofBlock — each requires its own concrete failure trace per WS-E methodology before addition.
