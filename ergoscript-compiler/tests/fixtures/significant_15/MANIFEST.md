...
|---|---|---|---|---|---|
| `chaincash_reserve.es`           | 611  | 609  | -2   | USED NODE | was -65 → -62 (S68) → -61 (S69 Append arm) → -60 (S70 Exponentiate arm) → +2 (S71 — `.multiply` lowering + `replace_all` MultiplyGroup arm) → **-2 (S73** — HIR `has_shared_field_in_clean_scope` anchor narrowing in `inline_single_use_vals`); chaincash compiles **with-segregation** (header 0x10 matches), RT-OK preserved. The +2 → -2 swing came from dropping the `noteTokenId` HIR anchor: when an FA's object ValDef has uses outside this val (so it survives at MIR as a graph-IR sym) AND the FA's smallest extractable wrapper appears in another sibling val's RHS in clean scope (so MIR CSE will extract it), the single-use val we're examining is doubly redundant as an anchor. For chaincash, `noteTokenId = noteInput.tokens(0)._1` and `noteValue` share `noteInput.tokens(0)` as the wrapper, and `noteInput` is multi-use → drop `noteTokenId`. `zBytes` keeps its anchor because `value.slice(33, value.size)` and `value.slice(0, 33)` aren't the same wrapper. Residual -2B: LOCAL still extracts `receiptOut.R6[Int].get` (MIR-level 2-use CSE) where NODE inlines it, and LOCAL still inlines `positionBytes = longToByteArray(position)` where NODE extracts it. The `receiptOut.R6` over-extraction is an MIR CSE choice, not an HIR anchor — out of scope for this approach. |
...
- `chaincash_reserve` (-2) — was -61 → +2 (S71) → **-2 (S73** — HIR anchor narrowing in `has_shared_field_in_clean_scope`). With-segregation header 0x10 matches, RT-OK preserved. S73 narrows the anchor when the FA's object ValDef is multi-use (survives at MIR) AND the FA's smallest extractable wrapper appears in another sibling val's RHS in clean scope (MIR CSE will extract that wrapper). Both gates together mean MIR has redundant anchors for the FA chain; the single-use val isn't load-bearing. Drops `noteTokenId` (`noteInput.tokens(0)` wrapper shared with `noteValue`). Doesn't drop `zBytes` (`value.slice(33, value.size)` and `value.slice(0, 33)` are different wrappers). Residual -2B: LOCAL still extracts `receiptOut.R6[Int].get` (MIR 2-use CSE) where NODE inlines, and LOCAL still inlines `positionBytes` where NODE extracts. Both are MIR CSE shape decisions, not HIR anchor problems — out of scope for S73. SigUSDV1 unchanged (still +15B). Known-future-arms in `replace_all` (unchanged): LongToByteArray, ByteArrayToLong, DecodePoint, GetVar, CreateProveDhTuple, Atleast, BitInversion, CalcSha256, CreateAvlTree, DeserializeContext, DeserializeRegister, ExtractBytesWithNoRef, FuncValue, SubstConstants, Xor, XorOf, ZkProofBlock — each requires its own concrete failure trace per WS-E methodology before addition.
