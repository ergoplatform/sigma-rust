...
|---|---|---|---|---|---|
| `chaincash_reserve.es`           | 611  | 613  | +2   | USED NODE | was -65 → -62 (S68) → -61 (S69 Append arm) → -60 (S70 Exponentiate arm) → **+2 (S71** — `.multiply` lowering + `replace_all` MultiplyGroup arm); chaincash now compiles **with-segregation** (header 0x10 matches). Residual +2B **is NOT constants-segment encoding** (the S72 hypothesis was falsified — both pools have identical multisets of 32 constants, just in different order, so segment-length is byte-equal). It IS a CSE-pipeline inlining-decision divergence in the inner `action == 0` block: LOCAL extracts 3 ValDefs that NODE inlines (`zBytes` 1×, `noteTokenId` 1×, `receiptOut.R6[Int].get` 2× CSE'd) and LOCAL inlines 1 ValDef that NODE extracts (`positionBytes = longToByteArray(position)` 2×). Net body diff: LOCAL inner-block 138B vs NODE 121B (+17), LOCAL trailing 263B vs NODE 278B (-15) → +2 total. The HIR `has_shared_field_in_clean_scope` anchor in `inline_single_use_vals` keeps `zBytes` (`value.slice` shared with `aBytes`) and `noteTokenId` (`noteInput.tokens` shared with `noteValue`), but by the END of MIR's CSE pipeline those two anchors are dead (count==0): an attempted S72 fix added count==0 dead-val drop to MIR's `inline_single_use_vals`, but it broke chaincash's segregation roundtrip (Const literals inside the dropped RHSs become orphaned in the constant-pool, falling back to no-seg at 493B). Reverted. **Defer to S73-class** — the right fix is a more thorough CSE-pipeline alignment that either prevents these anchors from becoming dead or compacts the constants pool after dead-val removal; chaincash and `SkyHarbor SigUSDV1` (+15B) likely share the same root cause and need a joint solve. |
...
- `chaincash_reserve` (+2) — was -61 fallback through S69/S70, **+2 with-segregation** post-S71. Header matches at 0x10. The S72 "constants-segment encoding drift" hypothesis was **falsified**: both pools have identical multisets of 32 constants in different order, so segment length is byte-equal. Real root cause is a CSE-pipeline inlining-decision divergence in the inner `action == 0` block: LOCAL extracts 3 ValDefs that NODE inlines (`zBytes` 1×, `noteTokenId` 1×, `receiptOut.R6[Int].get` 2×) and LOCAL inlines 1 ValDef that NODE extracts (`positionBytes` 2×). Inner block 138B (LOCAL) vs 121B (NODE) = +17B; trailing 263B vs 278B = -15B → net +2B. HIR's `has_shared_field_in_clean_scope` keeps `zBytes`/`noteTokenId` as scope anchors (RHS shares `value.slice`/`noteInput.tokens` FA with another val), but by the end of MIR's CSE pipeline those anchors are dead (count==0). The naive fix — adding count==0 drop to MIR's `inline_single_use_vals` — orphans Const literals from the dropped RHSs, breaking the constant-pool roundtrip and forcing chaincash to no-seg fallback at 493B. **Deferred to S73-class** as a joint solve with `SkyHarbor SigUSDV1` (+15, same shape: with-seg, RT-OK, anchor vals dead at end of CSE). Known-future-arms in `replace_all` (two removed): LongToByteArray, ByteArrayToLong, ~~Exponentiate~~, ~~MultiplyGroup~~, DecodePoint, GetVar, CreateProveDhTuple, Atleast, BitInversion, CalcSha256, CreateAvlTree, DeserializeContext, DeserializeRegister, ExtractBytesWithNoRef, FuncValue, SubstConstants, Xor, XorOf, ZkProofBlock — each requires its own concrete failure trace per WS-E methodology before addition.
