...
|---|---|---|---|---|---|
| `chaincash_reserve.es`           | 611  | 551  | -60  | USED NODE | was -65 → -62 (S68) → -61 (S69 Append arm; closed type-collision class) → -60 (**S70 partial — `replace_all` Exponentiate arm**; substituted dangling ValUse(26) under Exponentiate.right but chaincash still on no-seg fallback with new error `UnknownMethodId(MethodId(4), 7)` — `multiply` defined at sgroup_elem.rs:24 but absent from `METHOD_DESC` registry; deferred to S71) |
...
- `chaincash_reserve` (-60) — root cause is **constant-segregation roundtrip fallback**, not under-extraction. S69 added Append arm to `replace_all` (closed type-collision class — was `ValUse(13, Coll[Byte])` colliding with `history: SAvlTree`). S70 added Exponentiate arm (substituted dangling `ValUse(26, Coll[Byte])` buried as the `right` operand of Exponentiate via ByteArrayToBigInt → CalcBlake2b256 → Append-chain). Roundtrip now fails on a *different* class: `UnknownMethodId(MethodId(4), 7)` — `GroupElement.multiply` (and `exponentiate`, `exponentiateUnsigned`) are defined at [`sgroup_elem.rs`](../../../../ergotree-ir/src/types/sgroup_elem.rs#L70) but never appended to `METHOD_DESC` (only `getEncoded` + `negate` are registered). S71 entry point: register the missing method descriptors. Known-future-arms in `replace_all` (one removed): LongToByteArray, ByteArrayToLong, ~~Exponentiate~~, MultiplyGroup, DecodePoint, GetVar, CreateProveDhTuple, Atleast, BitInversion, CalcSha256, CreateAvlTree, DeserializeContext, DeserializeRegister, ExtractBytesWithNoRef, FuncValue, SubstConstants, Xor, XorOf, ZkProofBlock — each requires its own concrete failure trace per WS-E methodology before addition.
