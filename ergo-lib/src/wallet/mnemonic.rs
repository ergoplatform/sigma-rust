...
impl Mnemonic {
    ...
    pub fn generate(&self) -> Result<String, MnemonicError> {
        if Mnemonic::ALLOWED_STRENGTHS.contains(&self.strength) {
            ...
            Err(MnemonicError::InvalidStrength(self.strength))
        }
    }

    /// Generate mnemonic sentence using provided entrophy
    pub fn from_entrophy(&self, entrophy: Vec<u8>) -> Result<String, MnemonicError> {
        if !Mnemonic::allowed_entrophy_lens().contains(&entrophy.len()) {
            ...
            let phrase = entrophy_with_checksum
                .chunks(Mnemonic::BITS_GROUP_SIZE)
