...
mod tests {
    ...
    proptest! {
        ...
        fn eval_any(left in any::<EcPoint>(), pi in any::<DlogProverInput>()) {
            ...
            let right: BigInt256 = scalar_to_bigint256(pi.w);
    ...
    fn eval_exponent_greater_than_256_bit() {
        let left = force_any_val::<EcPoint>();
        let right = BigInt256::from_str_radix
            ("2240553423075396383515373673723462837462821468959827293462897346923874293642946928374923875928657983456938759287459236459287459238759346592837", 10)
            .unwrap();
        let expr: Expr = Exponentiate {
            left: Box::new(Expr::Const(left.into())),
            right: Box::new(Expr::Const(right.into())),
