...
fn _helper_extract_vecval(inp : Value) -> Result<Vec<Value>, EvalError> {
    match inp {
...
impl Evaluable for Append {
    fn eval(&self, env: &Env, ctx: &mut EvalContext) -> Result<Value, EvalError> {
        let input_vecval : Vec<Value> = _helper_extract_vecval(self.input.eval(env, ctx)?)?;
        let col_2_vecval : Vec<Value> = _helper_extract_vecval(self.col_2.eval(env, ctx)?)?;
        let concat_vecval : Vec<Value> = _helper_concat(input_vecval, col_2_vecval);
...
mod tests {
    ...
    fn test_append_basic_numbers() {
        for (test_inp, test_col2, expected_output) in [
                (vec![1,2,3,4], vec![5,6,7,8], vec![1,2,3,4,5,6,7,8]),
                (vec![], vec![], vec![]),
                (vec![], vec![1], vec![1]),
                (vec![], vec![2,2], vec![2,2]),
                (vec![1], vec![], vec![1]),
