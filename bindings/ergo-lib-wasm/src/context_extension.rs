...
pub struct ContextExtension(ergo_lib::ergotree_ir::sigma_protocol::prover::ContextExtension);

#[wasm_bindgen]
impl ContextExtension {
    /// Returns the number of elements in the collection
    pub fn len(&self) -> usize {
        let wrapped: ergo_lib::ergotree_ir::sigma_protocol::prover::ContextExtension =
            self.0.clone();
        wrapped.values.len()
    }
